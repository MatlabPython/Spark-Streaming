...
/** 正式部署供水的流量计采样小时级
  * Created by chiyong on 2017/4/9.
  */

object WaterFlowStreaming  {
  ...
  def createContext(prop: Properties) = {
    ...
    val groupid= prop.getProperty("kafka.groupid","waterflow")
    val interval = prop.getProperty("spark.interval","10").toInt
    val hbaseTable =prop.getProperty("hbase.table","WATER_FLOW")
    ...
    val checkpointPath = prop.getProperty("spark.checkpointPath","/tmp/streaming/checkpoint/waterFlow")


    val conf = new SparkConf().setAppName("WaterFlow")
    //注册kyro序列化
    ...
    //根据相同的Key聚合求最小的时间
    val os: DStream[(String, SensorDetailData)] = orders.reduceByKey((data1: SensorDetailData, data2: SensorDetailData) => {
    ...
    os.map(x => x._2).foreachRDD(rd => {
      rd.foreachPartition(recordPartitions => {
        insertBatchHBase("M", hbaseTable, recordPartitions, hbaseZookeeperPort, zookeeperquorum)
        logger.info("------------intoHbase---"+hbaseTable+"  success --------")
      })
    })
    ssc
  }
  ...
  /**
    ...
    * @param recordPartitions Iterator[SensorDetailData]迭代器，可于foreachPartition中使用
    * @param hbaseZookeeperPort hbase依赖的zookeeper端口
    * @param zookeeperquorum hbase依赖的zookeeper地址
    */
    ...
    try {
      ...
      for (data <- recordPartitions) {
        ...
        if (list.size() > 200) {
          mutator.mutate(list)
          mutator.flush()
          logger.info("----------------------flush 200 puts----------------------")
          list.clear()
        }

        putCount = putCount + 1;
      }
      logger.info("put count :" + putCount)
      mutator.mutate(list)
      mutator.flush()
      ...
      case ex: Exception => ex.printStackTrace()
        logger.error("insert into hbase error",ex)
    }
  }
