package com.gsafety.lifeline.bigdata.streaming.gas
...
/**
  ...
  * 燃气秒采样程序
  * 每秒取time时间最小的，
  * rowkey按秒进行覆盖，保证每个传感器每秒只有一条
  * 程序通过读取配置文件方式启动
  */
object GasStreaming {

  val logger = Logger.getLogger(GasStreaming.getClass);
  //采样时间
  var sample = 1
  //采样时间为秒
  ...
  def createContext(prop: Properties) = {
    ...
    val topic = prop.getProperty("kafka.topic","CLLSEN-GAS-HF-LOWSUMRY-PRO") //kafka topic
    val brokerList = prop.getProperty("kafka.broker.list","udap1:9092,udap5:9092")
    val groupid= prop.getProperty("kafka.groupid","gas_1s")
    val interval = prop.getProperty("spark.interval","10").toInt
  val hbaseTables =prop.getProperty("hbase.tables","GAS_1S,GasDetail").split(",")
    ...
    val checkpointPath = prop.getProperty("spark.checkpointPath","/tmp/streaming/checkpoint/gas1s")


    val conf = new SparkConf().setAppName("GasStreaming") //.setMaster("local[2]")
    //注册kyro序列化
  ...
  def insertBatchToHbase(cf: String, hbaseTable: String, recordPartitions: Iterator[SensorDetailData], hbaseZookeeperPort: String, zookeeperquorum: String): Unit = {
    try {
      ...
      val isSample=hbaseTable.equalsIgnoreCase("GAS_1S")
