package com.gsafety.lifeline.bigdata.streaming.drain
...
/**
  ...
  * Time: 16:22
  */
object DrainStreaming {

  val logger = Logger.getLogger(DrainStreaming.getClass);
  //采样时间
  var sample = 1
  //采样时间为秒
  ...
  def createContext(prop: Properties) = {
    ...
    val topic = prop.getProperty("kafka.topic","CLLSEN-DRAIN-HF-LOWSUMRY-PRO") //kafka topic
    val brokerList = prop.getProperty("kafka.broker.list","udap1:9092,udap5:9092")
    val groupid= prop.getProperty("kafka.groupid","drain_1s")
    val interval = prop.getProperty("spark.interval","10").toInt
    val hbaseTables =prop.getProperty("hbase.tables","DRAIN_1S,DRAIN").split(",")
    ...
    val checkpointPath = prop.getProperty("spark.checkpointPath","/tmp/streaming/checkpoint/drain1s")


    val conf = new SparkConf().setAppName("DrainStreaming") //.setMaster("local[2]")
    //注册kyro序列化
  ...
  def insertBatchToHbase(cf: String, hbaseTable: String, recordPartitions: Iterator[SensorDetailData], hbaseZookeeperPort: String, zookeeperquorum: String): Unit = {
    try {
      ...
      val isSample=hbaseTable.equalsIgnoreCase("DRAIN_1S")
